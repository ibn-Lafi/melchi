...
export type ReturnCustomer = { id: string; name: string; shopName: string | null };

// عملاء خط سير المندوب — RLS تُقيّد النتيجة تلقائيًا للعملاء المرتبطين به.
export async function getReturnCustomersAction(): Promise<ReturnCustomer[]> {
  const supabase = createSupabaseServerClient();
...
// حتى لا يتكرر إرجاع نفس البند أكثر من مرة.
