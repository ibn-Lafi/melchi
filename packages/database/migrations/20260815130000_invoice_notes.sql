-- ========== ملاحظات الفاتورة (اختيارية) ==========
-- المندوب يختار من ملاحظات جاهزة (مثل "لا يقبل المرتجع") أو يكتب نصًا حرًا
-- عند إصدار الفاتورة. تظهر بمستند الفاتورة (A4/PDF) بالأدمن وتطبيق المندوب.
alter table public.invoices
  add column if not exists notes text;

drop function if exists public.create_invoice_with_stock_check(
  uuid, uuid, jsonb, public.invoice_payment_method, numeric, uuid
);
