-- ========== سعر المنتج الذي يدخله الأدمن = شامل ضريبة القيمة المضافة ==========
-- قرار عمل: `products.price` / `product_units.unit_price` هو السعر النهائي
-- الذي يدفعه العميل فعليًا (شامل 15% ضريبة)، وليس سعرًا قبل الضريبة تُضاف
-- عليه لاحقًا. بالتالي عند إصدار الفاتورة: نستخرج السعر الصافي من السعر
-- الشامل (net = gross / 1.15) بدل إضافة الضريبة فوق السعر المُدخل — حتى
-- يبقى إجمالي الفاتورة النهائي مطابقًا تمامًا لسعر المنتج الذي حدّده الأدمن
-- (بعد الخصم إن وُجد)، وتظهر الضريبة كبند منفصل داخل نفس المبلغ وليس فوقه.
-- مثال: سعر المنتج بالأدمن = 100 → بالفاتورة: السعر الصافي 86.96 + ضريبة
-- 13.04 = 100 (بدون خصم).
--
-- لا تغيير على `products.price`/`product_units.unit_price` نفسها (تبقى كما
-- أدخلها الأدمن، شاملة الضريبة، وهذا ما يظهر بالمتجر العام كسعر معروض) —
-- التغيير فقط بكيفية تفكيكها داخل create_invoice_with_stock_check.
...
begin
  ...
  --    السعر الصافي وضريبته من داخل نفس المبلغ (gross / 1.15) — بدل إضافة
  --    ضريبة جديدة فوق السعر المُدخل.
  for v_item in select * from jsonb_array_elements(p_items)
  loop
    v_product_id := (v_item ->> 'product_id')::uuid;
    v_unit_id := (v_item ->> 'unit_id')::uuid;
